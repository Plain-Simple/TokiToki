...
public abstract class PartOfSpeechManipulator {
    ...
    public static String[] getArgs(String function_call) {
        ...
        for (char current_char : args_string.toCharArray()) {
            ...
            } else if (current_char == ' ' && args.get(current_arg_index).length() == 0) {
                skip = true;
            }
    ...
    public static String getFunctionName(String function_call) {
        return isMultiFunction(function_call) ? function_call.substring(0, function_call.indexOf('{')) : function_call.substring(0, function_call.indexOf('('));
    }
    ...
    public static String stripFromEnd(String original, String ending) {
        if (ending.equals(original.substring(original.length() - ending.length()))) {
    ...
    String[] breakRuleUp(String rule) {
        ...
        for (int i = 0; i < rule.length(); i++) {
            ...
            if (rule.length() - i >= 6 && rule.substring(i + 1, i + 6).equals(" and ") && p_level == 0 && c_level == 0 && !q_level) {
                rule_parts.add("");
                i += 5;
            }
        }
